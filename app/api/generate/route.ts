...
/**
 * Generate image using Google Imagen 4 via the Gemini API
 */
...
// Each code element maps to a hue on the full 360° color wheel.
// The colors in the prompt come from WHAT'S IN THE CODE, not the language.
const ELEMENT_COLORS: { key: keyof CodeMetrics; hue: number; name: string; hex: string }[] = [
...
// Additional accent colors injected randomly for variety
const WILD_ACCENTS = [
...
function getElementColorPalette(metrics: CodeMetrics): string {
  // Collect colors from elements that are actually present in the code
  const active: { name: string; hex: string; weight: number }[] = [];

  for (const ec of ELEMENT_COLORS) {
    const val = metrics[ec.key] as number;
    if (val > 0) {
      active.push({ name: ec.name, hex: ec.hex, weight: Math.min(val, 10) });
    }
  }

  // If code is too simple, give it a minimal palette
  if (active.length < 2) {
    active.push(
  ...
  // Always add 1-2 wild accents for unpredictability
  const shuffled = [...WILD_ACCENTS].sort(() => Math.random() - 0.5);
  const numAccents = active.length > 5 ? 2 : 1;

  // Sort by weight descending — dominant elements = dominant colors
...
const getDensityDirective = (lines: number): string => {
  if (lines > 500) return `
...
const getExtremityDirective = (metrics: CodeMetrics): string => {
...
const getArtMedium = (metrics: CodeMetrics): string => {
...
const getTextureOverlays = (metrics: CodeMetrics): string => {
...
const generateGalleryPrompt = (metrics: CodeMetrics) => {
  return `
...
interface CodeMetrics {
  ...
  unusedCodeLines: number;
  commentRatio: number;
  duplicateBlocks: number;
  magicNumbers: number;
}

function computeExtendedMetrics(code: string, language: string, signals: ReturnType<typeof analyzeCode>): CodeMetrics {
  ...
  // Count magic numbers (standalone numbers > 1 that aren't array indices [0], [1])
  ...
  // Detect duplicate 3-line blocks
  ...
  // Estimate unused code (commented-out code lines)
...
const generateDallePrompt = (metrics: CodeMetrics) => {
  ...
  // Compute chaos score for mark-making directive
  const chaos = metrics.cyclomaticComplexity +
    metrics.maxNestingDepth * 3 +
    metrics.loopCount;
...
function detectLanguage(code: string): string {
  ...
  // Default to javascript if nothing strong matched
  if (bestScore < 2) {
...
// Blood red gradient SVG for invalid/non-code input
const RED_GRADIENT_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024">
  <defs><radialGradient id="g" cx="50%" cy="50%" r="70%">
    <stop offset="0%" stop-color="#8b0000"/>
    <stop offset="60%" stop-color="#4a0000"/>
...
function looksLikeCode(code: string): boolean {
  ...
  const codePatterns = /\b(if|else|for|while|function|def|class|return|import|const|let|var|int|void|public|private|fn|match|struct|enum|async|await|try|catch|print|console)\b|[{}();=]|=>|->|::/g;
  const hits = (code.match(codePatterns) ?? []).length;
  return hits >= 2;
}

export async function POST(req: Request) {
  try {
    const { code, chaos } = await req.json();

    // Bound snippet so prompts remain stable + cheap.
    const codeSnippet = String(code ?? "").slice(0, 1800);

    // If input doesn't look like code, return blood red gradient
    if (!looksLikeCode(codeSnippet)) {
      const svgB64 = Buffer.from(RED_GRADIENT_SVG).toString("base64");
      return NextResponse.json({
        imageDataUrl: `data:image/svg+xml;base64,${svgB64}`,
        promptUsed: "Invalid input — blood red gradient fallback",
        signals: { language: "unknown", metrics: null },
      });
    }
    ...
    // Calculate metrics for the new prompt
    ...
    // Generate image (prompt already enforces no-text constraint)
    const b64 = await generateImageWithImagen(prompt);
